...
- Semantic, non-numeric tokens for everything else: surfaces (`canvas`, `surface`, `surface-sunken`, `surface-raised`, `surface-warm`), hairlines (`line`, `line-strong`), text from brightest to faintest (`ink-bright`, `ink`, `ink-soft`, `ink-muted`, `ink-dim`, `ink-faint`) and `on-accent` for text sitting on an accent fill. Every one of these clears WCAG AA (4.5:1) on every surface token.
...
**Carousel.** `src/components/Carousel.astro` owns the markup and loads behavior via `<script src='../carousel.js'>`. `src/carousel.js` is a plain DOM script (not a component) that `querySelector`s `.embla` and its children, wires the prev/next buttons, the dot row, the `01 / 10` snap counter, the pause/resume toggle (`.embla__autoplay`), and the Embla `Autoplay` plugin (loop on, 3s delay — skipped entirely when `prefers-reduced-motion: reduce`). It assumes exactly one `.embla` on the page.

The pause toggle is a WCAG 2.2.2 requirement, not decoration: auto-advancing content that starts on its own needs an explicit stop control. The button ships `hidden`; `addAutoplayToggle` reveals it only when the `Autoplay` plugin is actually present (so it stays hidden under reduced motion, where there is nothing to pause), and keeps its icon / `aria-pressed` / `aria-label` in sync via the `autoplay:play` / `autoplay:stop` events.
